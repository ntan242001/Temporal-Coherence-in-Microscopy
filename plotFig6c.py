...
fig.tight_layout(rect=[0.06, 0.07, 1, 1])

i=0
ax.plot(x_array*1e9, matrixIFN[:, i], 'r-',linewidth=width, label='FE')
ax.plot(x_array*1e9, matrixI1[:, i], 'k--',linewidth=width, label='Gaussian')
...
ax.set_yticklabels(yticks,fontsize=tick_font)

ax.set_ylabel('Intensity', fontsize=size)
ax.set_xlabel('Position (nm)', fontsize=size)

ax.text(-13.5, 1.09, '(c)',fontsize=size)
...
ax.axvline(find1684lines(True)[0]*1e9, color='r', linestyle='-',linewidth=vline_width)
ax.axvline(find1684lines(True)[1]*1e9, color='r', linestyle='-',linewidth=vline_width)
ax.axvline(find1684lines(False)[0]*1e9, color='k', linestyle='--',linewidth=vline_width)
ax.axvline(find1684lines(False)[1]*1e9, color='k', linestyle='--',linewidth=vline_width)
...
plt.savefig("Figure_6c.png", dpi=1000)
