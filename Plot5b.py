...
xmax = 15
ytop = 1.15
xticks = np.array([-15,-10,-5,0,5,10,15])
yticks = [0.5,1.0,1.5]

defocus_list = [-0.75,0.00,0.75,1.5,2.25]

fig, ax = plt.subplots(nrows=5, ncols=1, sharex=True)
fig.set_size_inches(4.55, 8)
fig.tight_layout(rect=[0.082, 0.05, 1.02, 1])
...
for i in range(5):
    ...
    ax[i].set_yticklabels(yticks,fontsize=tick_font)
    if i == 0: 
        continue
    ax[i].text(x=9.0, y=ytop, s = str(format(defocus_list[i], '.2f'))
...
ax1.text(x=4.9, y=ytop, s = '$\Delta z$ = ' + str(format(defocus_list[0], '.2f'))
         +r' $\mathrm{\mu}$'+'m', color = 'k', fontsize=size-4)

ax5.set_xticks(xticks)  
ax5.set_xticklabels(xticks,fontsize=tick_font)
ax3.set_ylabel('Intensity', fontsize=size+1, labelpad=8)
ax5.set_xlabel('Position (nm)', fontsize=size+1, labelpad=8)

ax1.text(-21.5, 1.5, '(b)',fontsize=size)
